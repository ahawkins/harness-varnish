...
class VarnishGaugeTest < MiniTest::Unit::TestCase
  ...
  def url ; 'http://foo.com/stats' ; end

  def setup
    super
    @gauge = Harness::VarnishGauge.new url
    @json = <<-json
      {
  ...
  def test_raises_an_error_if_request_fails
    assert_raises Harness::VarnishGauge::BadResponseError do
      stub_request(:get, 'http://bad.host').to_return({
        status: 404
      })

      Harness::VarnishGauge.new('http://bad.host').log
    end
  end
